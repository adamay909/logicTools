...
func _PrintDerivTree(lines []string, offset int) string {
	...
	lt = func(m *derivNode) (r string) {

		r = `[ .\p{` + m.line.seq.StringLatex() + `} `

		for _, e := range m.supportingLines {
...
func PrintDerivTree(lines []string, offset int) string {
	...
	return `\begin{forest}{for tree={grow=north}}` + "\n" + `%generated by gentzen` + "\n" + lt(dt) + `\end{forest}` + "\n\n"
